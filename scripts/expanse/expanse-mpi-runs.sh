#!/bin/bash
#SBATCH --job-name="mpi-stencil-2d"
#SBATCH --output="mpi-stencil-console-output.%j.%N.out"
...
# --cpus-per-task=, should be set to the number of threads you may want (pthread)
# --ntasks-per-node=, should be set to the number of cores you want (MPI) here we need a max of 16
